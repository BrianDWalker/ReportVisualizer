...
public final class HorizontalBarChartRenderer implements ChartRenderer {
    ...
    @Override public void render(GC gc, Rectangle bounds, ChartDataset data) {
        ...
        for (int c = 0; c < categories.size(); c++) {
            ...
            gc.drawText(label, Math.max(bounds.x + 4, plot.x - gc.textExtent(label).x - 8),
                    plot.y + c * slot + Math.max(0, (slot - gc.textExtent(label).y) / 2), true);
            for (int s = 0; s < series.size(); s++) {
                ...
                if (ChartDrawing.shouldDrawValueLabel(c, categories.size())) {
                    ...
                    gc.setForeground(gc.getDevice().getSystemColor(SWT.COLOR_LIST_FOREGROUND));
                    gc.drawText(value, Math.min(plot.x + plot.width - gc.textExtent(value).x,
                            plot.x + width + 5), y, true);
                }
            }
        }
        gc.drawText(data.yAxisTitle(), plot.x, bounds.y + 6, true);
    }
}
